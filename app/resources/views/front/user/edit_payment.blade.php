                ...
                <div class="container">
                   ...
                   <div class="row row-eq-height">
                      <div class="col-lg-6 col-xs-12 flush-left  xs-flush">
                        <form action="{{ route('user.payment.update',$payment_method->id) }}" id="update_method" method="post" enctype="multipart/form-data">
                        @csrf()
                            ...
                            <div class="iverify baccount">
                               <div class="form">
                                  <div class="form-group">
                                     ...
                                     <input type="text"  name="account_label" value="{{ $user_payment_method->account_label }}" placeholder="Enter Account Name">
                                  </div>
                                  @if($payment_method->name == 'Bank' || $payment_method->name == 'IMPS')
                                  <div class="form-group">
                                     <label>Bank account number</label>
                                     <input type="text"  name="account_number" value="{{ $user_payment_method->account_number }}" placeholder="Enter your Bank Account Number">
                                  </div>
                                  <div class="form-group">
                                     <label>Bank name</label>
                                     <input type="text"  name="bank_name" value="{{ $user_payment_method->bank_name }}" placeholder="Enter Bank Name">
                                  </div>
                                  <div class="form-group">
                                     <label>IFSC code</label>
                                     <input type="text"  name="ifs_code" value="{{ $user_payment_method->ifs_code }}" placeholder="Enter IFSC Code">
                                  </div>
                                  <div class="form-group">
                                     <label>Account Type (Optional)</label>
                                     <select  name="type" class="form-control">
                                        <option value="">Choose Account Type</option>
                                        <option value="current" {{  $user_payment_method->type == 'current' ? 'selected':'' }}>Current</option>
                                        <option value="saving" {{  $user_payment_method->type == 'saving' ? 'selected':'' }}>Saving</option>
                                     </select>
                                     {{-- <input type="text"  name="type" value="{{ $user_payment_method->type }}" placeholder="Specify the account type ( Current / Saving )"> --}}
                                  </div>
                                  @elseif($payment_method->name == 'UPI')
                                  <div class="form-group">
                                    <label>UPI ID</label>
                                    <input type="text"  name="account_number" value="{{ $user_payment_method->account_number }}" placeholder="Enter your UPI ID">
                                 </div>
                                 <div class="form-group">
